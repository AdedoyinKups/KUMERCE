...
const GenderItem = ({ category }) => {
  ...
  return (
    <button
      className={`${
        selected && "text-red"
      } group flex items-center justify-between ease-out duration-200 hover:text-red `}
    ...
    >
      ...
      <span
        className={`${
          selected ? "text-dark bg-red" : "bg-gray-2"
        } inline-flex rounded-[30px] text-custom-xs px-2 ease-out duration-200 group-hover:text-dark group-hover:bg-red`}
      >
        {category.products}
      </span>
    </button>
